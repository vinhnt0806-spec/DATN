...
function connectWebSocket() {
    ...
    console.log(`🔄 Đang thử kết nối tới WebSocket Server: ${socketUrl}`);
    updateDeviceStatusUI('connecting'); // 🆕 chưa biết ESP32 online/offline cho tới khi nhận được sync/device_status_changed
        ...
        try {
            ...
            switch (eventType) {
                ...
                case 'device_status':
                    ...
                    updateModeUI();
                    updateDeviceSwitches();
                    updateDeviceStatusUI('online'); // 🆕 nhận được device_status nghĩa là ESP32 đang online
                    break;
                ...
                case 'sync': 
                    ...
                    if (data.thresholds) thresholds = data.thresholds;
                    updateAllUI();
                    // 🆕 Gói sync gửi kèm deviceStatus -> dùng để hiển thị online/offline ngay khi Web vừa load/reconnect
                    if (data.deviceStatus) {
                        updateDeviceStatusUI(data.deviceStatus.online ? 'online' : 'offline');
                    }
                    break;
                ...
                case 'sensor': 
                    Object.assign(sensorData, data);
                    updateSensorUI();
                    updateDeviceStatusUI('online'); // 🆕 còn nhận được dữ liệu sensor nghĩa là ESP32 chắc chắn đang sống
                ...
                // 🆕 Server chủ động báo khi ESP32 online/offline thay đổi (xem server.js)
                case 'device_status_changed':
                    console.log(data.online ? "🟢 ESP32 vừa ONLINE" : "🔴 ESP32 vừa OFFLINE");
                    updateDeviceStatusUI(data.online ? 'online' : 'offline');
                    break;
            }
        } catch (err) {
            console.error("❌ Lỗi phân tích JSON từ Server:", err);
    ...
    ws.onclose = () => {
        console.log("🔴 Mất kết nối WebSocket! Sẽ tự động kết nối lại sau 3 giây...");
        // 🆕 Web mất kết nối tới Server thì không còn biết ESP32 đang thế nào nữa -> coi như "đang kết nối lại"
        updateDeviceStatusUI('connecting');
        if (!reconnectTimer) {
            reconnectTimer = setTimeout(connectWebSocket, 3000);
        }
    };
...
// 🆕 Cập nhật khối hiển thị trạng thái online/offline/đang kết nối ở sidebar
// state nhận 1 trong 3 giá trị: 'online' | 'offline' | 'connecting'
function updateDeviceStatusUI(state) {
    const wrapper = document.getElementById('systemStatus');
    const dot = document.getElementById('statusDot');
    const text = document.getElementById('statusText');
    if (!wrapper || !dot || !text) return;

    wrapper.classList.remove('online', 'offline', 'connecting');
    dot.classList.remove('online', 'offline', 'connecting');
    wrapper.classList.add(state);
    dot.classList.add(state);

    if (state === 'online') text.innerText = 'Thiết bị Online';
    else if (state === 'offline') text.innerText = 'Thiết bị Offline';
    else text.innerText = 'Đang kết nối...';
}
...
// 🆕 Kiểm tra trạng thái ESP32 ngay qua REST API, trước khi WebSocket kịp kết nối xong
// (giúp hiển thị đúng trạng thái gần như ngay lập tức khi vừa mở trang)
async function fetchInitialDeviceStatus() {
    try {
        const res = await fetch(`https://${BACKEND_URL}/api/device-status?t=${Date.now()}`, { cache: 'no-store' });
        const data = await res.json();
        updateDeviceStatusUI(data.online ? 'online' : 'offline');
    } catch (err) {
        console.warn("⚠️ Không lấy được trạng thái thiết bị ban đầu:", err);
    }
}
...
window.onload = function () {
    ...
    fetchInitialDeviceStatus(); // 🆕 hiển thị trạng thái ngay lập tức (trước khi WS kết nối xong)
