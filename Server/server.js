...
const authController = require("./controllers/authController"); // 🔥 Đường dẫn tới file authController của bạn
...
.then(async () => {
    console.log("🟢 Đã kết nối MongoDB");
    // 🔥 Gọi hàm tạo Admin từ authController thay vì tự viết ở đây
    await authController.createAdmin(); 
})
.catch((err) => console.log("❌ Lỗi kết nối MongoDB:", err));

// 2. KHỞI TẠO BỘ NHỚ RAM BAN ĐẦU
initMemoryStore();

// 3. ĐĂNG KÝ CÁC HTTP ROUTES
app.use("/", authRoutes);
app.use("/api", sensorRoutes); 

// 4. TẠO SERVER TỔNG HỢP HTTP & WEBSOCKET
const server = http.createServer(app);
const wss = new WebSocketServer({ server }); 

// 5. KÍCH HOẠT LOGIC WEBSOCKET TẬP TRUNG
